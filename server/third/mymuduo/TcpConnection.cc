...
TcpConnection::TcpConnection(EventLoop *loop,
    ...
    , localAddr_(loacalAddr)
    , peerAddr_(peerAddr)
    , highWaterMark_(64*1024*1024)
{
    // 下面给channel设置响应的回调函数，poller给channel通知感兴趣的事件发生了，channel会回调相应的函数
...
{
    if(!channel_->isWriting()) // 说明output中的数据已经发送完成
    {
...
void TcpConnection::handleClose()
{
    ...
    LOG_INFO << "fd=" << channel_->fd() << " state=" << (int)state_ << "\n";
    setState(kDisconnected);
    channel_->disableAll();
